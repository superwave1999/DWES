...
class Upload {
    ...
    function upload() {
        $result = false;
        if($this->error !== 1) {
            if($this->isValidSize() && $this->checkValidType()) {
                $this->error = 0;
    ...
    function checkValidType() {
        ...
        if($this->type === '') {
            ...
            $types[0] = strpos($tipo, $mimes[0]);
            $types[1] = strpos($tipo, $mimes[1]);
            $types[2] = strpos($tipo, $mimes[2]);
            $types[3] = strpos($tipo, $mimes[3]);
            
            
            for ($i = 0; $i < count($types); $i++) {
                if($types[$i] > 0) {
                    $this->uploadedtype = $mimes[$i];
                    $valid = true;
                }
